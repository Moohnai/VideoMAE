...
def train_one_epoch_BB(model: torch.nn.Module, data_loader: Iterable, optimizer: torch.optim.Optimizer,
    ...
    for step, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.no_grad():
            ...
            labels_mask[labels_mask_loc[0], labels_mask_loc[1], labels_mask_loc[2]] = 0.5
...
def train_one_epoch_BB_no_global_union(model: torch.nn.Module, data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, loss_scaler, max_norm: float = 0, patch_size: int = 16, 
                    normlize_target: bool = True, log_writer=None, lr_scheduler=None, start_steps=None,
                    lr_schedule_values=None, wd_schedule_values=None):
    model.train()
...
def train_one_epoch_BB_no_global_union_gradual(model: torch.nn.Module, data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, loss_scaler, max_norm: float = 0, patch_size: int = 16, 
                    normlize_target: bool = True, log_writer=None, lr_scheduler=None, start_steps=None,
